...
export class CamStreamSocketManager {
  ...
  static registerObserver( direction: CamStreamDirection, observer: CamStreamObserver ): void {
    ...
    if (!CamStreamSocketManager.observer[ctrl_id].hasOwnProperty(cmr_id)) {
      CamStreamSocketManager.observer[ctrl_id][cmr_id] = {};
    }

    if (!CamStreamSocketManager.observer[ctrl_id][cmr_id].hasOwnProperty(q)) {
  ...
  static async createProccess(direction: CamStreamDirection) {
    ...
    setTimeout(async () => {
      ...
      if (!CamStreamSocketManager.process[ctrl_id].hasOwnProperty(cmr_id)) {
        CamStreamSocketManager.process[ctrl_id][cmr_id] = {};
      }

      if (!CamStreamSocketManager.process[ctrl_id][cmr_id].hasOwnProperty(q)) {
        try {
