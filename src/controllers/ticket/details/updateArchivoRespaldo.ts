...
import { moveFile, toPosixPath } from '../../../utils/file';
...
export const updateArchivoRespaldo = asyncErrorHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    for (const archivo of archivosData) {
      const { nombreoriginal, ruta, tipo } = archivo;
      values.push(ruta, nombreoriginal, tipo, rt_id);
    }

    const placeholders = archivosData.map(() => `(?, ?, ?, ?)`).join(', ');

    const sql = `INSERT INTO nodo${ctrl_id}.archivoticket (ruta, nombreoriginal, tipo, rt_id) VALUES ${placeholders}`;
