...
export function getMulticastRtspStreamAndSubStream(rtspUriOnvif: string, username: string, password: string , manufacturer : "HIKVISION" | "Dahua"){
  ...
  return new Promise<string[]>((resolve, reject) => {
    ...
    if (!rtspUriOnvif.startsWith("rtsp://")) {
      let errRtsp = new Error('El enlace RTSP debe comenzar con "rtsp://"');
    ...
    if (manufacturer === "HIKVISION") {
      ...
      let rtspMain = rtspUriOnvif.split("?")[0];

      // Cambiar 101 --> 102
      let rtspSub = rtspMain.replace(/1(?=[^1]*$)/, "2");

      // Agregar credenciales al main rtsp
      let rtspMainWihtCredentials = addCredentialToRtsp(rtspMain,username,password);
    ...
    if(manufacturer === "Dahua"){
      ...
      let rtspMain = rtspUriOnvif.replace(/&unicast=true&proto=Onvif/,"");
    ...
    let errorManufacturer = new Error('Solo se soparta a los siguientes fabricantes: "HIKVISION" | "Dahua" ');
