...
export class Register {
  ...
  static getRegistros = handleErrorWithArgument<{ data: RowDataPacket[]; order_by: string }, GetRegisterProps>(async ({ ctrl_id, cursor, end_date, limit, start_date, type, p_action, ...rest }) => {
    const registerOption = RegisterConfigOptions[type as RegisterType];
    const otherQueryParams = Register.getRegisterFilterParamsAndValidate(type as RegisterType, rest);
    const { ordenation, relational } = Register.getRelationalOperatorAndOrdenation(p_action);
  ...
  static getRegistrosDownload = handleErrorWithArgument<{ data: RowDataPacket[]; columns: string[] }, GetRegisterDownload>(async ({ ctrl_id, end_date, start_date, type, col_delete, ...rest }) => {
    const registerOption = RegisterConfigOptions[type as RegisterType];
  ...
  private static getRegisterFilterParamsAndValidate(type_register: RegisterType, reqQuery: Record<string, string>) {
    ...
    const regisOption = RegisterConfigOptions[type_register];

    if (regisOption) {
      for (const q_key in reqQuery) {
  ...
  private static getPartialSqlQuery(filter_params: Record<string, string>) {
    ...
    const prevResult = resultQuery.join(' AND ').trim();
    const initialCondition = 'AND';
