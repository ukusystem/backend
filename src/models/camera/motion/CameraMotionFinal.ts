...
export class CameraMotionManager {
  ...
  static map: { [ctrl_id: number]: { [cmr_id: number]: CameraMotion } } = {};
    ...
    if (cam instanceof CameraMotion) {
      ...
      if (!exists) {
        ...
        if ( front_cmr_id != null && front_ctrl_id != null && front_ip != null && front_contraseña != null && front_usuario != null ) {
          const newCamMot = new CameraMotion({
            ip: front_ip,
  ...
  static deleteFfmpegProccess(cmr_id: number, ctrl_id: number) {
    ...
    console.log(`Eliminando Procesos Motion ctrl_id : ${ctrl_id} | cmr_id: ${cmr_id} `);
