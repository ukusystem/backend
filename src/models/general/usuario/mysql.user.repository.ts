...
export class MySQLUserRepository implements UserRepository {
  ...
  async findWithRoleAndPersonalById(u_id: number): Promise<UserWithRoleAndPersonal | undefined> {
    ...
    if (users[0] !== undefined) {
      const { usuario, fecha, apellido, nombre, p_id, rl_id, rol, u_uuid, activo } = users[0];
      const result: UserWithRoleAndPersonal = {
        u_id,
        u_uuid,
        usuario,
  ...
  async findByOffsetPagination(limit: number, offset: number): Promise<UserWithRoleAndPersonal[]> {
    ...
    const result = users.map<UserWithRoleAndPersonal>(({ apellido, fecha, nombre, p_id, rl_id, rol, u_id, usuario, activo, u_uuid }) => ({ activo, u_uuid, u_id, usuario, fecha, p_id, personal: { apellido, nombre, p_id }, rl_id, rol: { rl_id, rol } }));
    return result;
  }
  ...
  async create(data: CreateUserDTO): Promise<Usuario> {
    ...
    const result = await MySQL2.executeQuery<ResultSetHeader>({ sql: `INSERT INTO general.usuario ( u_uuid, usuario , contraseña , rl_id , p_id , fecha , activo ) VALUES ( ?, ? , ? , ? , ? , ? , 1 )`, values: [new_u_uuid, usuario, contraseña, rl_id, p_id, fecha] });
    return { usuario, contraseña, rl_id, p_id, activo: 1, fecha, u_id: result.insertId, u_uuid: new_u_uuid };
  }
