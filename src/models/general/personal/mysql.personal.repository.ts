...
export class MySQLPersonalRespository implements PersonalRepository {
  ...
  async updateByUuId(p_uuid: string, fieldsUpdate: UpdatePersonalDTO): Promise<void> {
    ...
    await MySQL2.executeQuery<ResultSetHeader>({ sql: `UPDATE general.personal SET ${queryValues.setQuery} WHERE p_uuid = ? LIMIT 1`, values: [...queryValues.setValues, p_uuid] });
  }
  async softDeleteByUuid(p_uuid: string): Promise<void> {
    await MySQL2.executeQuery<ResultSetHeader>({ sql: `UPDATE general.personal SET activo = 0 WHERE activo = 1 AND p_uuid = ? LIMIT 1`, values: [p_uuid] });
  }
  ...
  async create(data: CreatePersonalDTO): Promise<Personal> {
    ...
    const result = await MySQL2.executeQuery<ResultSetHeader>({
      sql: `INSERT INTO general.personal ( p_uuid, nombre, apellido, telefono, dni, co_id, foto, correo, c_id, activo ) VALUES ( ? , ? , ? , ? , ? , ? , ? , ? , 1 , 1 )`,
      values: [new_p_uuid, nombre, apellido, telefono, dni, co_id, foto, correo],
    });
    return { nombre, apellido, telefono, dni, co_id, foto, correo, c_id: 1, activo: 1, p_id: result.insertId, p_uuid: new_p_uuid };
  }
