...
export class SmartMap {
    ...
    static getControladorInfoByCtrlId = handleErrorWithArgument<ControladorInfo | null,{ctrl_id:number}>(async({ctrl_id})=>{
        ...
        const controladorInfo = await MySQL2.executeQuery<ControladorInfoRowData[]>({sql:`SELECT c.ctrl_id, c.nodo, c.rgn_id, r.region , c.direccion, c.descripcion, c.latitud, c.longitud , c.serie, c.ip , c.personalgestion , c.personalimplementador , c.conectado FROM general.controlador c INNER JOIN general.region r ON c.rgn_id = r.rgn_id WHERE c.activo = 1 AND c.ctrl_id = ?`,values:[ctrl_id]})
    ...
    static getCamerasInfoByCtrlId = handleErrorWithArgument<CameraInfo[] | [],{ctrl_id:number}>(async({ctrl_id})=>{
        ...
        const cameraData = await MySQL2.executeQuery<CameraInfoRowData[]>({sql:`SELECT c.cmr_id, c.serie, c.tc_id , t.tipo, c.m_id, m.marca, c.ip , c.descripcion, c.conectado FROM ${"nodo" + ctrl_id}.camara c INNER JOIN general.marca m ON c.m_id = m.m_id INNER JOIN general.tipocamara t ON c.tc_id = t.tc_id WHERE c.activo = 1`})
    ...
    static getEquiposEntradaByCtrlIdAndEquiEntId = handleErrorWithArgument<EquipoEntradaInfo[] | [], {ctrl_id:number, ee_id: number}>(async({ctrl_id, ee_id})=>{
        ...
        const equipEntr = await MySQL2.executeQuery<EquipoEntradaInfoRowData[]>({sql:`SELECT pe_id, pin, pe.ee_id , ee.detector , pe.descripcion, pe.estado FROM ${"nodo" + ctrl_id}.pinesentrada pe INNER JOIN general.equipoentrada ee ON pe.ee_id = ee.ee_id WHERE pe.activo =1 AND pe.ee_id = ?`,values:[ee_id]})
    ...
    static getEquiposSalidaByCtrlIdAndEquiSalId = handleErrorWithArgument<EquipoSalidaInfo[] | [], {ctrl_id:number, es_id: number}>(async({ctrl_id, es_id})=>{
        ...
        const equipSal = await MySQL2.executeQuery<EquipoSalidaInfoRowData[]>({sql:`SELECT ps_id, pin, ps.es_id , es.actuador , ps.descripcion, ps.estado FROM ${"nodo" + ctrl_id}.pinessalida ps INNER JOIN general.equiposalida es ON ps.es_id = es.es_id WHERE ps.activo =1 AND ps.es_id = ?`,values:[es_id]})
