...
const SolicitanteSchema = z.object({
    ...
    correo: z.string({required_error:"Solicitante: 'correo' es requerido",invalid_type_error:"'correo' debe ser un string"}).regex(/^[a-zA-Z0-9.!#$%&'*+\/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/,"El 'correo' ingresado es incorrecto.").max(100,"Máximo 100 caracteres permitidos para el 'correo'."),
