...
export const cameraControlSchema = z.object({
    ...
    action: z.enum(["start" ,"stop"],{errorMap: (_,ctx)=>{ return {message: "action : " + ctx.defaultError}}}),
    movement: z.enum(["Right" , "Left", "Up", "Down", "RightUp", "RightDown", "LeftUp", "LeftDown", "ZoomTele", "ZoomWide"],{errorMap: (_,ctx)=>{ return {message: "movement : " + ctx.defaultError}}}),
    velocity: z.union([z.string(), z.number()],{errorMap: ()=>{return {message: "'velocity' es requerido"}}}).pipe(z.coerce.number({required_error: "'velocity' es requerido",invalid_type_error: "'velocity' debe ser un numero"}).gte(0, "'velocity' debe ser mayor o igual a 0").lte(1,"'velocity' deber ser menor o igual a 1")),
})
