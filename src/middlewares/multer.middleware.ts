    ...
    upload(req, res, (err) => {
      ...
      if (err instanceof multer.MulterError) {
        if (err.code === 'LIMIT_FILE_COUNT') {
          const filesPermitedCount = limitsMulter?.files ?? 'no_especificado';
          ...
          const formDataFieldPermited = fields.map((fieldItem) => fieldItem.name);
          if (req.files) deleteTemporalFiles(req.files); // limpiar
          ...
          const limitFileSizePermited = limitsMulter?.fileSize ?? 'no_especificado';
          if (req.files) deleteTemporalFiles(req.files); // limpiar
